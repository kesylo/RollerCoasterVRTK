...
Copyright 2015-2018 Zen Fulcrum LLC
...
- You can uniformly scale track to a size of your preference. Non-uniform scaling is not supported.
...
- Due to limitations/bugs with the Unity Editor's undo/redo system, operations that remove track pieces (such as deleting an object or merging several pieces) might not be properly linked together after an undo. You'll have to use "Snap to Nearest" when the links get broken. Sorry. :-(
- Internally carts use a 1kg guide mass on their sliding joints. If you plan to have a cart that weighs much more or less than that, you may need to tweak this value to keep the physics simulation stable. The rigidbody is set up in TrackCart.SetupGuide.
...
Also note: Unity sometimes imports .fbx models with a non-identity scale (depending on the software/settings when you exported with). Make sure that the "scale" for any imported .fbx models is set to 1 unless you have something else in mind.
